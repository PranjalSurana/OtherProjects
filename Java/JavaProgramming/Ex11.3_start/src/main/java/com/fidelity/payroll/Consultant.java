...
public class Consultant extends Employee {
	...
	private void setContractAmount(BigDecimal contractAmount) {
		Objects.requireNonNull(contractAmount, "Employee contract amount must be positive");
		if (contractAmount.compareTo(BigDecimal.ZERO) <= 0) {
			throw new IllegalArgumentException("Employee contract amount must be positive");
		}
	...
	private void setContractLengthInMonths(int contractLengthInMonths) {
		if (contractLengthInMonths <= 0) {
			throw new IllegalArgumentException("Employee contract length must be positive");
		}
