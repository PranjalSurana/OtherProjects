...
class AccountPortalTest {
	...
	void addAccountTest_Failure_LimitExceeded() {
		...
		Account newAccount1 = new DebitAccount("acc8", new BigDecimal(12000), new BigDecimal(26000));
		accountPortalTestforAddAccount.addAccount(newAccount1);
		Account newAccount2 = new DebitAccount("acc0", new BigDecimal(12000), new BigDecimal(1600));
	...
	void getPortfolioTest_Success() {
		assertEquals(3, accountPortal.getMaximumNumberOfAccounts());
	}
	...
	void removeAccountTest_Success_AccountPresent() {		
		...
		assertEquals(2, portfolioTest.size());
	}
