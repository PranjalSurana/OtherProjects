...
class GicAccountTest {
	...
	void calculateCurrentBalanceTest_LessValue() {
		...
		BigDecimal currBalance = gicAccountTest.calculateCurrentBalance();
		assertEquals(new BigDecimal("10300"), currBalance);
	}
	
	@Test
	@DisplayName("calculateCurrentBalanceTest: Value is greater than Fee")
	void calculateCurrentBalanceTest_CurrentDayAfterStartDate() {
		gicAccountTest = new GicAccount("acc2", new BigDecimal(1000), new BigDecimal(7), LocalDate.of(2024, 11, 1), 6, new BigDecimal(900));
		BigDecimal currBalance = gicAccountTest.calculateCurrentBalance();
