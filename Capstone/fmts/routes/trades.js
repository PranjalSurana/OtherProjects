...
router.get("/instrument/:instrumentId", (request, response, next) => {
  ...
  try {
    ...
    let instrument = instrumentList.find(

      (instrument) => instrument.instrumentId === request.params.instrumentId

    );

 

    response.end(JSON.stringify(instrument));
