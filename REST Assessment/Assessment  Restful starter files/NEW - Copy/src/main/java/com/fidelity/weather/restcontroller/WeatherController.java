...
public class WeatherController {
    ...
    @Autowired
    private WeatherDao weatherService;
    ...
    public ResponseEntity<WeatherForecast> getWeatherForecast(@PathVariable("city") String city, @PathVariable("countryCode") String countryCode, @PathVariable("regionCode") String regionCode) {
        ...
        try {  
            ...
            LatitudeAndLongitude latitudeAndLongitude = weatherService.getLatitudeAndLongitude(city, countryCode, regionCode);
            ...
            if (latitudeAndLongitude != null) {
                ...
                logger.warn("getForecast: " + city + ", " + countryCode, regionCode + " not found");
                forecastResponse = ResponseEntity.badRequest().build();
            }
            logger.debug("getForecast({}, {}, {}): return {}", city, countryCode, regionCode, forecastResponse);
            return forecastResponse;
        }
        catch (WeatherDatabaseException e) {
            logger.error("getForecast({}, {}): {}", city, countryCode, regionCode, e.getMessage());
    ...
    private WeatherForecast getForecastFromBackend(float latitude, float longitude) {
